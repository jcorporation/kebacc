...
void send_data_to_rrdcached(struct mg_mgr *mgr, enum wallbox_report report, struct t_wallbox_response *data) {
    ...
    if (line == NULL) {
        KEBACC_LOG_ERROR("JSON parsing failure");
        return;
    }
    KEBACC_LOG_DEBUG("Sending %s", line);
    ...
    if (mg_user_data->rrdached_conn == NULL) {
        ...
        if (mg_user_data->rrdached_conn == NULL) {
            KEBACC_LOG_ERROR("Error creating connection to rrdached: %s", config->rrdcached_uri);
            FREE_PTR(line);
        }
    }
    else {
...
static void ev_handler_rrdcached(struct mg_connection *nc, int ev, void *ev_data) {
    ...
    if (ev == MG_EV_CONNECT) {
        ...
        KEBACC_LOG_ERROR("Connection error occurred");
    }
